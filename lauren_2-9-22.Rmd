---
title: "Add Weights, Effect Across Counties"
author: "Blain Morin"
date: "2/9/2022"
...
# Redo checkpoint-off states with weights
...
iplot(drunks.model, 
      xlab = 'Time to treatment (Years)',
      main = 'Event study: Total Fatal Creashes with Alcohol Detected')
...
df.sum = df %>%
  ...
  summarise(DUI_Total = sum(dui_tot_arrests),
            ...
            county_population = mean(population, na.rm = TRUE)) %>%
  ungroup()
...
temp = df3 %>%
  select(state, fips_county_code, county_population) %>%
  group_by(state, fips_county_code) %>%
  summarise(population = mean(county_population, na.rm = TRUE))
...
df5 %>%
  filter(population > 0) %>%
  ggplot(aes(y = delta, x = log(population))) +
  geom_point(alpha = .6) + 
  facet_wrap(~state) +
  ylim(-4, 1) +
  theme_bw() +
...
* Bit of upward trend in Texas, but negative or flat elsewhere
...
df5 %>%
  ...
  theme_bw() +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle("County Population Versus Delta Proportion Black DUI")

```

* I do not see an obvious trend
...
df5 %>%
  ...
  theme_bw() +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle("County Population Versus Delta n Fatal Crashes")
